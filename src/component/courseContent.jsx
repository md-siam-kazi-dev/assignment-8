...
const CourseContent = () => {
    ...
    const Params = useSearchParams(); // This hook triggers the error in image_62c28a.jpg
    ...
    const handleSearch = () => {
       const formattedSearch = searchText.toLowerCase().trim();
       setSearchText(formattedSearch);
       router.push('/', { scroll: false });
       setSearchInput(formattedSearch);
       setcategoryMain('all');
    }
    ...
    return (
        <div className="container-div mt-20 text-center">
            ...
            <div className="btn-container flex-wrap flex gap-1">
                {categories.map((cate) => (
                    <button 
                        onClick={() => handleClick(cate.slug)} 
                        key={cate.id} 
                        className={`btn ${categoryName === cate.slug ? 'btn-primary text-white' : 'btn-outline'} rounded-full border-primary md:text-[16px] text-[12px] text-primary border-1`}
                    >
                        {cate.name}
                    </button>
                ))}
            ...
            <Field orientation="horizontal" className='w-full border rounded-full p-2 shadow mx-auto mt-5 mb-5 md:w-7/10 lg:w-5/10 xl:w-4/10'>
                <Input type="search" value={searchText} placeholder="Search Course" onChange={handleInput} />
